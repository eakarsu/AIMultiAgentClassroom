...
// Route: /api/gap-assignments
// Feature: Assignments
// Description: Assignments / submission system
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Assignments. Assignments / submission system' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'assignments', title: 'Assignments', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'assignments', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['assignments']);
      return res.json({ slug: 'assignments', items: r.rows || [] });
    }
    return res.json({ slug: 'assignments', items: [] });
  } catch (e) {
    return res.json({ slug: 'assignments', items: [], error: (e && e.message) || 'Server error' });
  }
});
