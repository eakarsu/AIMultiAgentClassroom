...
// Route: /api/gap-parent
// Feature: Parent
// Description: Parent communication channel
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Parent. Parent communication channel' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'parent', title: 'Parent', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'parent', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['parent']);
      return res.json({ slug: 'parent', items: r.rows || [] });
    }
    return res.json({ slug: 'parent', items: [] });
  } catch (e) {
    return res.json({ slug: 'parent', items: [], error: (e && e.message) || 'Server error' });
  }
});
