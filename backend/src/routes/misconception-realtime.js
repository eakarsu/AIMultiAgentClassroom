// Auto-generated feature route: misconception-realtime
// Domain: real-time misconception detector — Capability: adapting lesson path live
// Mounted under: /api/misconception-realtime
...
// POST /api/misconception-realtime/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a real-time misconception detector. Your task is adapting lesson path live. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/misconception-realtime/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/misconception-realtime/:id — fetch one
...
// DELETE /api/misconception-realtime/:id — discard a run
