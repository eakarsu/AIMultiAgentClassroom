// Auto-generated feature route: socratic-dialogue
// Domain: Socratic dialogue agent — Capability: guiding learners via questioning
// Mounted under: /api/socratic-dialogue
...
// POST /api/socratic-dialogue/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a Socratic dialogue agent. Your task is guiding learners via questioning. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/socratic-dialogue/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/socratic-dialogue/:id — fetch one
...
// DELETE /api/socratic-dialogue/:id — discard a run
