// Auto-generated feature route: multi-modal-tutor
// Domain: multi-modal adaptive tutor — Capability: tutoring with text + voice + image and adaptive difficulty
// Mounted under: /api/multi-modal-tutor
...
// POST /api/multi-modal-tutor/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a multi-modal adaptive tutor. Your task is tutoring with text + voice + image and adaptive difficulty. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/multi-modal-tutor/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/multi-modal-tutor/:id — fetch one
...
// DELETE /api/multi-modal-tutor/:id — discard a run
