// Auto-generated feature route: path-personalizer
// Domain: learning path personalizer — Capability: personalizing paths by strength/weakness
// Mounted under: /api/path-personalizer
...
// POST /api/path-personalizer/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a learning path personalizer. Your task is personalizing paths by strength/weakness. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/path-personalizer/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/path-personalizer/:id — fetch one
...
// DELETE /api/path-personalizer/:id — discard a run
