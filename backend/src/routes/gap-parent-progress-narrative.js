...
// Route: /api/gap-parent-progress-narrative
// Feature: Parent Progress Narrative
// Description: (auto-summary for parents)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Parent Progress Narrative. (auto-summary for parents)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Parent Progress Narrative\nDescription: (auto-summary for parents)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['parent-progress-narrative', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'parent-progress-narrative', title: 'Parent Progress Narrative', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'parent-progress-narrative', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['parent-progress-narrative']);
      return res.json({ slug: 'parent-progress-narrative', items: r.rows || [] });
    }
    return res.json({ slug: 'parent-progress-narrative', items: [] });
  } catch (e) {
    return res.json({ slug: 'parent-progress-narrative', items: [], error: (e && e.message) || 'Server error' });
  }
});
