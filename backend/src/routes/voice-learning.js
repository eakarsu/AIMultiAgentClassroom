// Auto-generated feature route: voice-learning
// Domain: voice-enabled learning agent — Capability: supporting accent-neutral ASR-based interactions
// Mounted under: /api/voice-learning
...
// POST /api/voice-learning/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a voice-enabled learning agent. Your task is supporting accent-neutral ASR-based interactions. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/voice-learning/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/voice-learning/:id — fetch one
...
// DELETE /api/voice-learning/:id — discard a run
