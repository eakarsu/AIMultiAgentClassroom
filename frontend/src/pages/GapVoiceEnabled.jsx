...
// Voice Enabled
// Voice-enabled learning with accent-neutral ASR

export default function GapVoiceEnabled() {
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-voice-enabled', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 960, margin: '0 auto' }}>
      <h1 style={{ fontSize: 28, marginBottom: 6 }}>Voice Enabled</h1>
      <p style={{ color: '#555', marginBottom: 20 }}>Voice-enabled learning with accent-neutral ASR</p>
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, padding: 20, marginBottom: 20 }}>
        ...
        <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
          ...
          >
            {loading ? 'Running...' : 'Run Voice Enabled'}
          </button>
          <button
